...
class TranslatableJsonSqlTest extends TestCase
{
    ...
    public function test_call_sites_do_not_order_or_pluck_translatable_json(): void
    {
        ...
        $categoryFilter = substr($postsTable, (int) strpos($postsTable, "SelectFilter::make('categories')"));
        $categoryFilter = substr($categoryFilter, 0, (int) strpos($categoryFilter, "SelectFilter::make('author')"));
        $this->assertStringContainsString("orderBy('sort_order')", $categoryFilter);
        $this->assertStringContainsString('whereHas(', $categoryFilter);
        $this->assertStringNotContainsString('->relationship(', $categoryFilter);
